...
// GET /jobs/:id — protected (company-scoped)
...
// GET /public/jobs/:id — public (open jobs only)
export const getPublicJobById = async (req, res, next) => {
  try {
    const job = await getJobService(Number(req.params.id), { publicOnly: true });
    res.status(200).json(job);
  } catch (error) {
    next(error);
  }
...
export const deleteJob = async (req, res, next) => {
  try {
    await deleteJobService(Number(req.params.id), req.user.companyId);
